...
def create_catchment(fpath,
    ...
    dem, info, pos, cellsize, nodata = read_AsciiGrid(os.path.join(fpath, f'dem_{rsl}/orig_dem_0.asc'))
    cmask, _, _, _, _ = read_AsciiGrid(os.path.join(fpath, f'dem_{rsl}/cmask_d8_10.asc'))
    flowacc, _, _, _, _ = read_AsciiGrid(os.path.join(fpath, f'dem_{rsl}/acc_d8_0.asc'))
    flowpoint, _, _, _, _ = read_AsciiGrid(os.path.join(fpath, f'dem_{rsl}/fdir_d8_0.asc'))
    slope, _, _, _, _ = read_AsciiGrid(os.path.join(fpath, f'dem_{rsl}/slope_d8_0.asc'))
    twi, _, _, _, _ = read_AsciiGrid(os.path.join(fpath, f'dem_{rsl}/twi_d8_0.asc'))
...
def create_pristine_catchment(fpath, 
                     set_non_forest_as='null',
                     plotgrids=True, 
                     plotdistr=False):
    """
    ...
    # drained spruce and pine mires to be filled with 'pristine' information
    pristpeat = {'spruce_mire': {
                    'stand_density': 0.2865, # [-] Gustavsen and Päivänen 1986
                    'vol': 1e-4*78.25, # [m3/m2] Gustavsen and Päivänen 1986
                    'p_vol': 0.16*1e-4*78.25, # [m3/m2] Gustavsen and Päivänen 1986
                    's_vol': 0.54*1e-4*78.25, # [m3/m2] Gustavsen and Päivänen 1986
                    'b_vol': 0.30*1e-4*78.25, # [m3/m2] Gustavsen and Päivänen 1986
                    'ba': 1e-4*15.25, # [m2/m2] # Gustavsen and Päivänen 1986
                    'height': 10, # [m] Gustavsen and Päivänen 1986
                    'cf': np.nan, # calculated later Korhonen et al. 2007
                    'age': 68, # Gustavsen and Päivänen 1986
                    'diameter': 0.122, # [m] Gustavsen and Päivänen 1986
                    'LAIpine': np.nan, 
                    'LAIspruce': np.nan,
                    'LAIdecid': np.nan,
                    'ditch': 0.0}, # [m] ditch depth
                 'pine_mire': {
                    'stand_density': 0.2110, # [-] Gustavsen and Päivänen 1986
                    'vol': 1e-4*47, # [m3/m2] Gustavsen and Päivänen 1986
                    'p_vol': 0.67*1e-4*78.25, # [m3/m2] Gustavsen and Päivänen 1986
                    's_vol': 0.15*1e-4*78.25, # [m3/m2] Gustavsen and Päivänen 1986
                    'b_vol': 0.18*1e-4*78.25, # [m3/m2] Gustavsen and Päivänen 1986        
                    'ba': 1e-4*9.67, # [m2/m2] # Gustavsen and Päivänen 1986
                    'height': 9, # [m] Gustavsen and Päivänen 1986
                    'cf': np.nan, # calculated later Korhonen et al. 2007
                    'age': 84, # Gustavsen and Päivänen 1986
                    'diameter': 0.124, # [m] Gustavsen and Päivänen 1986
                    'LAIpine': np.nan,
                    'LAIspruce': np.nan,
                    'LAIdecid': np.nan,
                    'ditch': 0.0} # [m] ditch depth
    ...
    # third drained peatlands to estimated 'pristine' values
    convert_drained_peatlands_pristine = True
    if convert_drained_peatlands_pristine == True:
        cd[ix_sm] = pristpeat['spruce_mire']['diameter']
        vol[ix_sm] = pristpeat['spruce_mire']['vol']
        p_vol[ix_sm] = pristpeat['spruce_mire']['p_vol']
        s_vol[ix_sm] = pristpeat['spruce_mire']['s_vol']
        b_vol[ix_sm] = pristpeat['spruce_mire']['b_vol']
        ba[ix_sm] = pristpeat['spruce_mire']['ba']
        height[ix_sm] = pristpeat['spruce_mire']['height']
        cf[ix_sm] = cf_spruce_model_alt(ba=pristpeat['spruce_mire']['ba']*1e4, 
                                        a=pristpeat['spruce_mire']['age'], 
                                        no=pristpeat['spruce_mire']['stand_density']*1e4)
        age[ix_sm] = pristpeat['spruce_mire']['age']
        #bmleaf_pine[ix_sm] = pristpeat['spruce_mire']['diameter']
        #bmleaf_spruce[ix_sm]= pristpeat['spruce_mire']['diameter']
        #bmleaf_decid[ix_sm]= pristpeat['spruce_mire']['diameter']
        LAI_pine[ix_sm] = stem_volume_to_LAI_mVMI_fit(V=pristpeat['spruce_mire']['p_vol']*1e4, tree='pine')
        LAI_spruce[ix_sm] = stem_volume_to_LAI_mVMI_fit(V=pristpeat['spruce_mire']['s_vol']*1e4, tree='spruce')
        LAI_decid[ix_sm] = stem_volume_to_LAI_mVMI_fit(V=pristpeat['spruce_mire']['b_vol']*1e4, tree='decid')
        stand_density[ix_sm] = pristpeat['spruce_mire']['stand_density']
        stream[ix_sm] = pristpeat['spruce_mire']['ditch']
        
        cd[ix_pm] = pristpeat['pine_mire']['diameter']
        vol[ix_pm] = pristpeat['pine_mire']['vol']
        p_vol[ix_pm] = pristpeat['pine_mire']['p_vol']
        s_vol[ix_pm] = pristpeat['pine_mire']['s_vol']
        b_vol[ix_pm] = pristpeat['pine_mire']['b_vol']
        ba[ix_pm] = pristpeat['pine_mire']['ba']
        height[ix_pm] = pristpeat['pine_mire']['height']
        cf[ix_pm] = cf_pine_model_comp(ba=pristpeat['pine_mire']['ba']*1e4,
                                       ch=pristpeat['pine_mire']['height'],
                                       a=pristpeat['pine_mire']['age'], 
                                       no=pristpeat['pine_mire']['stand_density']*1e4)
        age[ix_pm] = pristpeat['pine_mire']['age']
        #bmleaf_pine[ix_pm] = pristpeat['spruce_mire']['diameter']
        #bmleaf_spruce[ix_pm]= pristpeat['spruce_mire']['diameter']
        #bmleaf_decid[ix_pm]= pristpeat['spruce_mire']['diameter']
        LAI_pine[ix_pm] = stem_volume_to_LAI_mVMI_fit(V=pristpeat['pine_mire']['p_vol']*1e4, tree='pine')
        LAI_spruce[ix_pm] = stem_volume_to_LAI_mVMI_fit(V=pristpeat['pine_mire']['s_vol']*1e4, tree='spruce')
        LAI_decid[ix_pm] = stem_volume_to_LAI_mVMI_fit(V=pristpeat['pine_mire']['b_vol']*1e4, tree='decid')
        stand_density[ix_pm] = pristpeat['pine_mire']['stand_density']
        stream[ix_pm] = pristpeat['pine_mire']['ditch']
...
def cf_spruce_model_alt(ba, a, no):
    '''
    Based on Korhonen et al. 2007: Local models for forest canopy cover with beta regression

    ba = basal area [m2/ha]
    a  = age [years]
    no = number of stems [ha-1]
    '''
    n = (-2.4709 + 0.12843*ba - 0.003597*ba**2 + 6.4329*1e-5*ba**3 + 0.13252*a - 
    0.0027546*a**2 + 1.5183*1e-5*a**3 + 0.00010652*no)
    cf = (np.exp(n))/(1+np.exp(n))
    return cf

def cf_pine_model_comp(ba, ch, a, no):
    '''
    Based on Korhonen et al. 2007: Local models for forest canopy cover with beta regression
    ...
    no = number of stems [ha-1]
    '''
    n = (-1.5392 + 0.19772*ba - 0.0039876*ba**2 + 4.3117*1e-5*ba**3 - 0.032476*a + 
    2.486*1e-4*a**2 - 5.3266*1e-7*a**3 + 0.16375*ch - 0.011887*ch**2 + 1.9104*1e-4*ch**2 + 6.3301*1e-5*no)
    cf = (np.exp(n))/(1+np.exp(n))
    return cf
